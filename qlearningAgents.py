...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if possibleActions:
          for pAction in possibleActions:
            maxQ = max(maxQ, self.getQValue(state, pAction))
          return maxQ
        return 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        if possibleActions:
          for pAction in possibleActions:
            qVal = self.getQValue(state, pAction)
            if (qVal > maxQ):
              maxQ = qVal
              bestAction = pAction
          return bestAction
    ...
    def update(self, state, action, nextState, reward):
        ...
        newQ += self.alpha*(reward + self.discount * (self.computeValueFromQValues(nextState)))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feat in features:
          ...
          difference = (reward + self.discount*(maxQ)) - currQ

          tmp_weights[feat] = self.weights[feat] + self.alpha * difference * features[feat]
